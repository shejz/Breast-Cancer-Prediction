...
# Cancer Identification using Neural Networks
#
# Neural Networks
# 01. Multi-Layer Perceptron (mlp)              - Binary & MultiClass Classification
# 02. Multi-Layer Perceptron with SGD (mlpSGD)  - Binary & MultiClass Classification
# 03. Deep Neural Network (deepnet)             - Binary & MultiClass Classification
# 04. Neural Network (nnet)                     - Binary & MultiClass Classification
...
dataSet <- read.csv("BreastCancerWisconsin.data.csv", 
                    header = FALSE, sep = ',')
...
# 01. Multi-Layer Perceptron (mlp)  - Binary & MultiClass Classification
fit.mlp_1 <- caret::train(Class~., data=training_dataset[2:11], method="mlp", 
                          metric="Accuracy", trControl=control)
print(fit.mlp_1)
#plot(fit.mlp_1)

# 02. Multi-Layer Perceptron with SGD (mlpSGD)  - Binary & MultiClass Classification
fit.mlpSGD_1 <- caret::train(Class~., data=training_dataset[2:11], method="mlpSGD", 
                             metric="Accuracy", trControl=control)
print(fit.mlpSGD_1)
#plot(fit.mlpSGD_1)

# 03. Deep Neural Network (deepnet) - Binary & MultiClass Classification
fit.deepnet_1 <- caret::train(Class~., data=training_dataset[2:11], method="dnn", 
                              metric="Accuracy", trControl=control)
print(fit.deepnet_1)
#plot(fit.deepnet_1)

# 04. Neural Network (nnet) - Binary & MultiClass Classification
fit.nnet_1 <- caret::train(Class~., data=training_dataset[2:11], method="nnet", 
                           metric="Accuracy", trControl=control)
print(fit.nnet_1)
#plot(fit.nnet_1)
...
# 01. Multi-Layer Perceptron (mlp)  - Binary & MultiClass Classification
fit.mlp_2 <- caret::train(Class~., data=training_dataset[2:11], method="mlp", 
                          metric="Accuracy", trControl=control,
...
# 02. Multi-Layer Perceptron with SGD (mlpSGD)  - Binary & MultiClass Classification
fit.mlpSGD_2 <- caret::train(Class~., data=training_dataset[2:11], method="mlpSGD", 
                             metric="Accuracy", trControl=control,
                             preProcess = c('center', 'scale','pca'))
print(fit.mlpSGD_2)
#plot(fit.mlpSGD_2)
...
# 03. Deep Neural Network (deepnet) - Binary & MultiClass Classification
fit.deepnet_2 <- caret::train(Class~., data=training_dataset[2:11], method="dnn", 
                              metric="Accuracy", trControl=control,
                              preProcess = c('center', 'scale','pca'))
print(fit.deepnet_2)
#plot(fit.deepnet_2)
...
# 04. Neural Network (nnet) - Binary & MultiClass Classification
fit.nnet_2 <- caret::train(Class~., data=training_dataset[2:11], method="nnet", 
                           preProcess = c('center', 'scale','pca'), 
                           metric="Accuracy", trControl=control)
print(fit.nnet_2)
#plot(fit.nnet_2)
...
# 01. Multi-Layer Perceptron (mlp)  - Binary & MultiClass Classification
fit.mlp_3 <- caret::train(Class~., data=training_dataset[2:11], method="mlp", 
                          metric="Accuracy", trControl=control,
                          preProcess = c('center', 'scale','pca'),
                          tuneLength = 3)
print(fit.mlp_3)
...
# 02. Multi-Layer Perceptron with SGD (mlpSGD)  - Binary & MultiClass Classification
fit.mlpSGD_3 <- caret::train(Class~., data=training_dataset[2:11], method="mlpSGD", 
                             metric="Accuracy", trControl=control,
                             preProcess = c('center', 'scale','pca'),
                             tuneLength = 3)
print(fit.mlpSGD_3)
...
# 03. Deep Neural Network (deepnet) - Binary & MultiClass Classification
fit.deepnet_3 <- caret::train(Class~., data=training_dataset[2:11], method="dnn", 
                              metric="Accuracy", trControl=control,
                              preProcess = c('center', 'scale','pca'),
                              tuneLength = 3)
print(fit.deepnet_3)
...
# 04. Neural Network (nnet) - Binary & MultiClass Classification
fit.nnet_3 <- caret::train(Class~., data=training_dataset[2:11], method="nnet", 
                           preProcess = c('center', 'scale','pca'), 
                           metric="Accuracy", trControl=control, 
                           tuneLength = 3)
print(fit.nnet_3)
...
# -----------------------------------------------------------------------------
## 4. Training - with explicit parameter tuning using preProcess method 
## & Manual Grid i.e. tuneGrid
## Training using Neural Network Algorithms
# -----------------------------------------------------------------------------

# Neural Network (nnet)
grid <- expand.grid(size=c(seq(from = 1, to = 50, by = 2)),
                    decay=c(seq(from = 0.0, to = 0.3, by = 0.05)))
fit.nnet_4 <- caret::train(Class~., data=training_dataset[2:11], method="nnet", 
                           preProcess = c('center', 'scale','pca'), 
                           metric="Accuracy", trControl=control,
                           tuneGrid = grid)
print(fit.nnet_4)
...
results <- resamples(list(
  MLP_1       = fit.mlp_1,
  MLPSGD_1    = fit.mlpSGD_1,
  Nnet_1      = fit.nnet_1,
  Deepnet_1   = fit.deepnet_1,
  
  MLP_2       = fit.mlp_2,
  MLPSGD_2    = fit.mlpSGD_2,
  Nnet_2      = fit.nnet_2,
  Deepnet_2   = fit.deepnet_2,                          
  
  MLP_3       = fit.mlp_3,
  MLPSGD_3    = fit.mlpSGD_3,
  Nnet_3      = fit.nnet_3,
  Deepnet_3   = fit.deepnet_3,                          
...
predictions_nnet_3 <- predict(fit.nnet_3, newdata=testing_dataset)

# Evaluate the BEST trained model and print results
res_nnet_3  <- caret::confusionMatrix(predictions_nnet_3, testing_dataset$Class)

print("Results from the BEST trained model ... ...\n"); 
print(res_nnet_3) 
print(round(res_nnet_3$overall, digits = 3))

## Save the BEST trained model to disk
final_model <- fit.nnet_3;        saveRDS(final_model, "./final_model_nnet_3.rds")
...
trained_model <- readRDS("./final_model_nnet_3.rds")
