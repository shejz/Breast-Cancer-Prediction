...
colnames(dataSet) <- c('SampleCodeNumber', 
                       ...
                       'NormalNucleoli', 
                       'Mitoses',
                       'Class') # Class: 2 for benign and 4 for malignant
...
# 01. Random Forest : cforest - Binary & MultiClass Classification
fit.cForest_1 <- caret::train(Class~., data=training_dataset[2:11], method="cforest", 
                              metric="Accuracy", trControl=control)
print(fit.cForest_1)
#plot(fit.cForest_1)

# 02. Random Forest : parRF   - Binary & MultiClass Classification
fit.parRF_1 <- caret::train(Class~., data=training_dataset[2:11], method="parRF", 
                            metric="Accuracy", trControl=control)
print(fit.parRF_1)
#plot(fit.parRF_1)

# 03. Bagged CART : treebag   - Binary & MultiClass Classification
fit.treebag_1 <- caret::train(Class~., data=training_dataset[2:11], method="treebag", 
                              metric="Accuracy", trControl=control)
print(fit.treebag_1)
#plot(fit.treebag_1)

# 04. Randon Forest : RRF - Binary & MultiClass Classification 
fit.rRF_1 <- caret::train(Class~., data=training_dataset[2:11], method="RRF", 
                          metric="Accuracy", trControl=control)
print(fit.rRF_1)
#plot(fit.rRF_1)
...
# 01. Random Forest : cforest - Binary & MultiClass Classification
fit.cForest_2 <- caret::train(Class~., data=training_dataset[2:11], method="cforest", 
                              metric="Accuracy", trControl=control,
...
# 02. Random Forest : parRF   - Binary & MultiClass Classification
fit.parRF_2 <- caret::train(Class~., data=training_dataset[2:11], method="parRF", 
                            metric="Accuracy", trControl=control,
                            preProcess = c('center', 'scale','pca'))
print(fit.parRF_2)
#plot(fit.parRF_2)
...
# 03. Bagged CART : treebag   - Binary & MultiClass Classification
fit.treebag_2 <- caret::train(Class~., data=training_dataset[2:11], method="treebag", 
                              metric="Accuracy", trControl=control,
                              preProcess = c('center', 'scale','pca'))
print(fit.treebag_2)
#plot(fit.treebag_2)
...
# 04. Randon Forest : RRF - Binary & MultiClass Classification 
fit.rRF_2 <- caret::train(Class~., data=training_dataset[2:11], method="RRF", 
                          metric="Accuracy", trControl=control,
                          preProcess = c('center', 'scale','pca'))
print(fit.rRF_2)
#plot(fit.rRF_2)
...
# 01. Random Forest : cforest - Binary & MultiClass Classification
fit.cForest_3 <- caret::train(Class~., data=training_dataset[2:11], method="cforest", 
                              metric="Accuracy", trControl=control,
                              preProcess = c('center', 'scale','pca'),
                              tuneLength = 3)
print(fit.cForest_3)
...
# 02. Random Forest : parRF   - Binary & MultiClass Classification
fit.parRF_3 <- caret::train(Class~., data=training_dataset[2:11], method="parRF", 
                            metric="Accuracy", trControl=control,
                            preProcess = c('center', 'scale','pca'),
                            tuneLength = 3)
print(fit.parRF_3)
...
# 03. Bagged CART : treebag   - Binary & MultiClass Classification
fit.treebag_3 <- caret::train(Class~., data=training_dataset[2:11], method="treebag", 
                              metric="Accuracy", trControl=control,
                              preProcess = c('center', 'scale','pca'),
                              tuneLength = 3)
print(fit.treebag_3)
...
# 04. Randon Forest : RRF - Binary & MultiClass Classification 
fit.rRF_3 <- caret::train(Class~., data=training_dataset[2:11], method="RRF", 
                          metric="Accuracy", trControl=control,
                          preProcess = c('center', 'scale','pca'),
                          tuneLength = 3)
print(fit.rRF_3)
...
predictions_parRF_3 <- predict(fit.parRF_3, newdata=testing_dataset)

# Evaluate the BEST trained model and print results
res_parRF_3  <- caret::confusionMatrix(predictions_parRF_3, testing_dataset$Class)

print("Results from the BEST trained model ... ...\n"); 
print(res_parRF_3) 
print(round(res_parRF_3$overall, digits = 3))

## Save the BEST trained model to disk
final_model <- fit.parRF_3;        saveRDS(final_model, "./final_model_parRF_3.rds")
...
trained_model <- readRDS("./final_model_parRF_3.rds")
