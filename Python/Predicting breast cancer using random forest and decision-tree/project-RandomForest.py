...
def evaluateAlgorithm(train_X, test_X, train_Y, test_Y, dataset):
    ...
    # -------------------------------------------------------------------        
    RF_1 = RandomForestClassifier() # Parameter Settings (default)
    model = RF_1
    ...
    model_List.append(('RF_1', 
                       'Random Forest Algorithm: PS-1', 
                        trained_Model, acc, kappa_score, 
                        classReport, confMatrix))

    # -------------------------------------------------------------------
    # PS - Parameter Settings - (manual tuning of "n_estimators", 
    # "criterion", "max_depth" )
    # -------------------------------------------------------------------
    # Algorithm initialisation - PS - 2    
    # ------------------------------------------------------------------- 
    RF_2 = RandomForestClassifier(n_estimators=1000, criterion='gini', 
                                  max_depth=8, 
                                  min_samples_split=2, 
                                  min_samples_leaf=1, 
    ...
    # collect performance results for further reporting
    model_List.append(('RF_2', 
                       'Random Forest Algorithm: PS-2', 
                        trained_Model, acc, kappa_score, 
                        classReport, confMatrix))    
    ...
    # ------------------------------------------------------------------- 
    RF_3 = RandomForestClassifier(n_estimators=500, criterion='entropy', 
                                  max_depth=4, 
                                  min_samples_split=2, 
                                  min_samples_leaf=1, 
                                  min_weight_fraction_leaf=0.0, 
    ...
    # collect performance results for further reporting
    model_List.append(('RF_3', 
                       'Random Forest Algorithm: PS-3', 
                        trained_Model, acc, kappa_score, 
                        classReport, confMatrix)) 
    ...
    # collect performance results for further reporting
    model_List.append(('RF_4', 'Randon Forest Algorithm: PS-4', 
                        trained_Model, acc, kappa_score, 
                        classReport, confMatrix))    
...
def featureRank_Analysis(model, pred_dataset, cols):
    ...
    tree.export_graphviz(
            ...
            model.steps[1][1].estimators_[sub_tree_number], 
            out_file = dotfile, 
