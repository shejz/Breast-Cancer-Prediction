...
def loadDataSetFrMySQLTable():
    ...
    dataset.ix[dataset.Class == 4, 'NumericalClass'] = 1
    dataset.ix[dataset.Class == 2, 'NumericalClass'] = 0
...
def summariseDataset(dataset):
    ...
    colNumeric.remove('NumericalClass');
...
def preProcessingData(dataset):
    ...
    # Separate out Numerical & Categorical Variables
    colNumeric = []; colCategory = []; target = 'NumericalClass';
    colNames = dataset.columns.values.tolist()
    ...
    # --------------------------    
    cols_X = colNumeric     
    cols_X.remove('Class'); cols_X.remove('NumericalClass');
    print(cols_X)
...
def evaluateAlgorithm(train_X, test_X, train_Y, test_Y, dataset):
    ...
    # -------------------------------------------------------------------
    # Explore xgboost Algorithm from XGBoost
    # -------------------------------------------------------------------
    # 2. Gradient Boosting i.e. xgboost Algorithm 
    # -------------------------------------------------------------------
    # XGBoost can handle Numerical Class or Label
    # 1. Data Cleaning
    # Separate out Numerical & Categorical Variables
    colNumeric = []; colCategory = []; target = 'NumericalClass';
    colNames = dataset.columns.values.tolist()
    ...
    # Convert Year (numerical) Cols to Categorical Cols
    dataset[['Class']] = dataset[['Class']].astype('int64')
    ...
    # Algorithm initialisation - PS - 1
    XGBoost_1 = xgb.XGBClassifier(gamma=0.5, 
                    learning_rate=0.01, 
                    max_delta_step=0.1, 
                    max_depth=8, 
    ...
    xgmat = xgb.DMatrix(xgb_train_X, label=xgb_train_Y)
    param = {}
    ...
    # Evaluate the skill of the Trained model
    xgmat = xgb.DMatrix(xgb_test_X)
    pred_Class          = trained_Model.predict(xgmat)
    acc                 = accuracy_score(xgb_test_Y, pred_Class)
    classReport         = classification_report(xgb_test_Y, pred_Class)
    confMatrix          = confusion_matrix(xgb_test_Y, pred_Class) 
    kappa_score         = cohen_kappa_score(xgb_test_Y, pred_Class) 
    ...
    XGBoost_3 = xgb.XGBClassifier(gamma=0.5, 
                    learning_rate=0.15, max_delta_step=0, 
                    max_depth=6, min_child_weight=1, 
                    n_estimators=800, nthread=4, 
    ...
    grid = GridSearchCV(estimator=model, param_grid = parameters,
                        cv = KFold)
    grid.fit(xgb_train_X, xgb_train_Y)
...
def featureRank_Analysis(model, pred_dataset, cols):
    ...
    # Get an arbitary tree number 
    #xgb.plot_tree(model.steps[1][1], num_trees = 7, #rankdir='LR' 
    #             )
    #pyplot.savefig('tree.pdf')
    #xgb.plot_importance(model.steps[1][1])
    #pyplot.savefig('features.pdf')
...
def loadTrainedModelForPrediction(dataset):
    f = open('model_XGBoost_5.pickle', 'rb')
